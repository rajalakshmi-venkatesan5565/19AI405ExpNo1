...
class MedicineAgent:
    ...
    def act(self, environment):
        # Get the patient's temperature
        temp = environment[self.location]["temperature"]

        # Check if the patient is unhealthy
        if temp > 98.5:
            print(
                f"Patient in {self.location} unhealthy "
                f"({temp:.1f}°F). Giving medicine."
            )
            
            # Increase performance after treatment
            self.performance += 10
            
            # Set temperature to normal after treatment
            environment[self.location]["temperature"] = 98.0
        else:
            print(
                f"Patient in {self.location} healthy "
                f"({temp:.1f}°F)."
            )

        # Move to the other room
        self.location = (
            "Room B" if self.location == "Room A" else "Room A"
        )

        # Decrease performance for movement
        self.performance -= 1
...
def main():
    ...
    # Create the medicine prescribing agent
    agent = MedicineAgent()
    ...
    for step in range(5):
        ...
        # Agent checks and treats the patient
        agent.act(environment)

        # Randomly make a patient unhealthy for the next check
        random_room = random.choice(["Room A", "Room B"])
        environment[random_room]["temperature"] = random.uniform(
            99.0, 103.0
        )
